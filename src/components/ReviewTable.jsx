  ...
  if (currentType === "BAD") {
    return (
      <div className="grid gap-2">
        <select
          className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm outline-none transition focus:border-pine-500 focus:ring-2 focus:ring-pine-100"
          value={badReasonValue}
...
function TypeButtons({ row, currentType, onTypeChange }) {
  return (
...
function getRowTone(row, draft, isSelected) {
  ...
  return {
    currentType,
    wrapperClass: isSelected ? `${toneClass} ring-2 ring-inset ring-yellow-300` : toneClass
  };
}
...
export default function ReviewTable({ loading, displayRows, drafts, badReasonModes, selectedRowId, rowKey, onSelect, onTypeChange, onBadReasonModeChange, onReasonChange }) {
  ...
  return (
    <>
      <div className="grid gap-4 lg:hidden">
        {displayRows.map((row) => (
          <MobileCard
            key={row.id}
            row={row}
            draft={drafts[rowKey(row.id)]}
            badReasonMode={badReasonModes[rowKey(row.id)]}
            isSelected={selectedRowId === String(row.id)}
            onSelect={onSelect}
      ...
      <div className="hidden overflow-auto rounded-2xl border border-slate-200 bg-slate-50/80 lg:block">
